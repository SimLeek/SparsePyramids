...
class PyramidGroup2D:
        ...
        for i in range(len(self.sizes)):
            for t in self.tensors:
                if t.shape[-2] > self.sizes[i][0] and t.shape[-1] > self.sizes[i][1]:
                    self.transformed_inputs[i].append(
                        ((t.shape[-2], t.shape[-1]), self.transforms[i](t))
                    )
                else:
                    continue
    ...
    def apply(x: Union["PyramidGroup2D", torch.Tensor], conv: torch.nn.Conv2d):
        ...
        initial_conv_size = torch.Size(
            [
                1 + (k - 1) * d
                for k, d in zip(x.conv_args["kernel_size"], x.conv_args["dilation"])
            ]
        )
        for t in x.tensors:
...
def test_apply_resize():
    from sparsepyramids.tests import larg
    import PIL.Image
    import numpy as np
